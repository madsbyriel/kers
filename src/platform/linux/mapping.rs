...
impl TryFrom<Key> for evdev::KeyCode {
    ...
    fn try_from(value: Key) -> Result<Self, Self::Error> {
        ...
        Ok(match value {
            ...
            Key::Digit9 => K::KEY_9,
            Key::Digit0 => K::KEY_0,
            Key::Minus => K::KEY_MINUS,
            Key::Equal => K::KEY_EQUAL,
            Key::Backspace => K::KEY_BACKSPACE,
            Key::Tab => K::KEY_TAB,
            Key::Q => K::KEY_Q,
            Key::W => K::KEY_W,
            Key::E => K::KEY_E,
            ...
            Key::O => K::KEY_O,
            Key::P => K::KEY_P,
            Key::LeftBracket => K::KEY_LEFTBRACE,
            Key::RightBracket => K::KEY_RIGHTBRACE,
            Key::Enter => K::KEY_ENTER,
            Key::A => K::KEY_A,
            Key::B => K::KEY_B,
            Key::C => K::KEY_C,
            Key::D => K::KEY_D,
            ...
            Key::K => K::KEY_K,
            Key::L => K::KEY_L,
            Key::Semicolon => K::KEY_SEMICOLON,
            Key::Apostrophe => K::KEY_APOSTROPHE,
            Key::Grave => K::KEY_GRAVE,
            Key::Backslash => K::KEY_BACKSLASH,
            Key::Z => K::KEY_Z,
            Key::X => K::KEY_X,
            Key::V => K::KEY_V,
            Key::N => K::KEY_N,
            Key::M => K::KEY_M,
            Key::Comma => K::KEY_COMMA,
            Key::Dot => K::KEY_DOT,
            Key::Slash => K::KEY_SLASH,
            Key::IntlBackslash => K::KEY_102ND,
            Key::Space => K::KEY_SPACE,
            Key::LeftCtrl => K::KEY_LEFTCTRL,
            Key::RightCtrl => K::KEY_RIGHTCTRL,
            Key::LeftShift => K::KEY_LEFTSHIFT,
            Key::RightShift => K::KEY_RIGHTSHIFT,
            Key::LeftAlt => K::KEY_LEFTALT,
            Key::RightAlt => K::KEY_RIGHTALT,
            Key::LeftMeta => K::KEY_LEFTMETA,
            Key::RightMeta => K::KEY_RIGHTMETA,
            Key::Compose => K::KEY_COMPOSE,
            Key::Fn => K::KEY_FN,
            Key::Menu => K::KEY_MENU,
            Key::CapsLock => K::KEY_CAPSLOCK,
            Key::NumLock => K::KEY_NUMLOCK,
            Key::ScrollLock => K::KEY_SCROLLLOCK,
            Key::Insert => K::KEY_INSERT,
            Key::Delete => K::KEY_DELETE,
            Key::Home => K::KEY_HOME,
            Key::End => K::KEY_END,
            Key::PageUp => K::KEY_PAGEUP,
            Key::PageDown => K::KEY_PAGEDOWN,
            Key::Up => K::KEY_UP,
            Key::Down => K::KEY_DOWN,
            Key::Left => K::KEY_LEFT,
            Key::Right => K::KEY_RIGHT,
            Key::PrintScreen => K::KEY_SYSRQ,
            Key::Pause => K::KEY_PAUSE,
            Key::F1 => K::KEY_F1,
            Key::F2 => K::KEY_F2,
            Key::F3 => K::KEY_F3,
            Key::F4 => K::KEY_F4,
            ...
            Key::F23 => K::KEY_F23,
            Key::F24 => K::KEY_F24,
            Key::Numpad0 => K::KEY_KP0,
            Key::Numpad1 => K::KEY_KP1,
            Key::Numpad2 => K::KEY_KP2,
            Key::Numpad3 => K::KEY_KP3,
            Key::Numpad4 => K::KEY_KP4,
            Key::Numpad5 => K::KEY_KP5,
            Key::Numpad6 => K::KEY_KP6,
            Key::Numpad7 => K::KEY_KP7,
            Key::Numpad8 => K::KEY_KP8,
            Key::Numpad9 => K::KEY_KP9,
            Key::NumpadEnter => K::KEY_KPENTER,
            Key::NumpadPlus => K::KEY_KPPLUS,
            Key::NumpadMinus => K::KEY_KPMINUS,
            Key::NumpadAsterisk => K::KEY_KPASTERISK,
            Key::NumpadSlash => K::KEY_KPSLASH,
            Key::NumpadDot => K::KEY_KPDOT,
            Key::NumpadEqual => K::KEY_KPEQUAL,
            Key::NumpadComma => K::KEY_KPCOMMA,
            Key::KpJpComma => K::KEY_KPJPCOMMA,
            Key::MouseLeft => K::BTN_LEFT,
            Key::MouseRight => K::BTN_RIGHT,
            Key::MouseMiddle => K::BTN_MIDDLE,
            Key::MouseSide => K::BTN_SIDE,
            Key::MouseExtra => K::BTN_EXTRA,
            Key::MouseForward => K::BTN_FORWARD,
            Key::MouseBack => K::BTN_BACK,
            other => {
                return Err(crate::Error::KeyNotMapped(other));
            }
        })
    }
}
...
mod tests {
    ...
    const MAPPINGS: &[(u16, Key)] = &[
        ...
        (evdev::KeyCode::KEY_S.code(), Key::S),
